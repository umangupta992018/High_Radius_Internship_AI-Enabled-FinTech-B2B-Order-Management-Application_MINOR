...
function App() {
  ...
  const fetchData = async () => {
    const response = await axios
      .get(
        `http://localhost:8080/1806336/InvoiceServlet?page=${pageCount}`
      )
      .catch((error) => {
        console.log(error);
      });    
        ...
        <TableBody>
          {responseData.map((data,index) => (
            <TableRow className={classes.tablerow} id='trow' hover role="checkbox" key={index.toString()}>
              ...
              <TableCell width="12%" className={classes.tablecell} align="right">{data.amo}</TableCell>
              <TableCell width="15%" className={classes.tablecell} align="left">{data.due_date}</TableCell>
              ...
              <TableCell width="25%" className={classes.tablecell} align="left">{data.note}</TableCell>
            </TableRow>
          ))}
        </TableBody>
        </Table>
